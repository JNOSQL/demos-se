...
image::https://www.jnosql.org/img/logos/couchdb.png[CouchDB, width=200px]
...
image::http://www.jnosql.org/img/logos/couchbase.svg[Couchbase, width=200px]
...
QuestDB exposes its Web Console and HTTP services on port `9000`. The Eclipse JNoSQL driver also connects through this endpoint.
...
The JNoSQL QuestDB test configuration uses this same URL format and the `qdb` database.
