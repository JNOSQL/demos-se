...
This sample demonstrates Eclipse JNoSQL Time Series support using an account transaction scenario. Each transaction is modeled as a time-based event where the `Instant` identifier represents the point in time at which the transaction occurred.

The sample shows two programming models:
...
The sample represents account activity as a sequence of transactions over time.
...
The transaction status is represented by an enum:
...
The `Instant` identifier represents the time dimension of the transaction.

==== TimeSeriesTemplate

The `TimeSeriesTemplate` API provides direct access to time-series persistence operations.

[source,java]
----
public class App {

    private static final AccountTransaction TRANSACTION =
    ...
    public static void main(String[] args) {
        ...
        try (SeContainer container =
            ...
            Optional<AccountTransaction> transaction =
                    template.find(
                            TRANSACTION.getId(),
...
The same entity can also be accessed using Jakarta Data repositories.
...
The repository can then be injected from the CDI container using the Time Series database qualifier:
        ...
        try (SeContainer container =
            ...
            AccountTransactionRepository repository =
                    container.select(
                            ...
                            DatabaseQualifier.ofTimeSeries()
                    ).get();
