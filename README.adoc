...
image::https://www.jnosql.org/img/logos/valkey.png[Valkey, width=200px]

Valkey is an open-source in-memory key–value database, used as a distributed cache and message broker, with optional durability. Because it holds all data in memory and because of its design, Valkey offers low-latency reads and writes, making it particularly suitable for use cases that require a cache.

==== How to install with Docker

1. Install docker: https://www.docker.com/
1. https://hub.docker.com/r/valkey/valkey
1. Run docker command:

[source, bash]
----
docker run --name valkey-instance -p 6379:6379 -d valkey/valkey:latest
----

==== Projects

* link:valkey/[Valkey sample]
...
image::https://www.jnosql.org/img/logos/influxdb.png[InfluxDB, width=200px]

InfluxDB is a time-series database designed to store and query data that changes over time.

This sample demonstrates Eclipse JNoSQL Time Series support with InfluxDB using both:
...
NOTE: This tutorial starts InfluxDB with `--without-auth` to keep the local setup simple. Therefore, no authentication token needs to be generated. The token property is provided as a placeholder for the InfluxDB driver configuration.

==== Optional: Explore the data in the browser

InfluxDB 3 Explorer provides a browser-based interface for querying and visualizing data stored in InfluxDB 3 Core.

[source,bash]
----
docker run -d --name influxdb3-explorer -p 8888:8080 influxdata/influxdb3-ui:1.9.0
----
...
Configure Explorer to connect to:
...
and use the `metrics` database.

==== Projects

* link:influxdb/[InfluxDB Time Series sample]

=== Apache IoTDB

image::https://www.jnosql.org/img/logos/apache-io-tbd.png[Apache IoTDB, width=200px]

Apache IoTDB is a time-series database designed for IoT data management and analysis.

This sample demonstrates Eclipse JNoSQL Time Series support with Apache IoTDB using both:
...
Apache IoTDB will be available through its native RPC endpoint at:
...
NOTE: For the Docker-based tutorial, `jnosql.iotdb.enable.redirection=false` is recommended because the address advertised by the container may not be reachable through the mapped host port.

==== Projects

* link:iotdb/[Apache IoTDB Time Series sample]

=== QuestDB

image::https://www.jnosql.org/img/logos/questdb.png[QuestDB, width=200px]
...
QuestDB exposes its Web Console and HTTP services on port `9000`. The Eclipse JNoSQL driver also connects through this endpoint. :contentReference[oaicite:1]{index=1}
...
No database creation step is required for this sample.
...
The `jnosql.timeseries.database` property defines the logical database used by the Time Series manager.
...
The JNoSQL QuestDB test configuration uses this same URL format and the `qdb` database. :contentReference[oaicite:2]{index=2}
...
From the SQL editor, you can inspect the sensor data written by Eclipse JNoSQL.
...
Or retrieve the latest reading for a specific sensor:
...
* link:questdb/[QuestDB Time Series sample]
